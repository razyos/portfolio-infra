...
prepare_argocd_values() {
    # Retrieve secrets
    ARGOCD_ADMIN_PASSWORD=$(get_secret "portfolio/argocd-admin-password")
    GITHUB_SSH_KEY=$(get_secret "portfolio/github-ssh-key")
    ...
    GITHUB_SSH_KEY_FORMATTED=$(echo "$GITHUB_SSH_KEY" | sed '/^sshPrivateKey: |/d' | sed 's/^/        /')
    ...
    awk -v admin_pwd="$ARGOCD_ADMIN_PASSWORD_QUOTED" \
        -v repo_url="$GITHUB_REPO_URL" \
        -v argocd_host="$ARGOCD_HOST_LOWER" \
        -v ssh_key="$GITHUB_SSH_KEY_FORMATTED" '
    BEGIN { in_ssh_key = 0 }
